...
class SliderCrankMechanism(Scene):
    ...
    def construct(self):
        ...
        rect_ancla = Square(side_length=0.4).set_stroke(None, 0).set_fill(self.ancla_color, 1).move_to(
            O2.get_center() + DOWN * semi_circulo_ancla.get_width() / 2)

        ancla = VGroup(semi_circulo_ancla, rect_ancla)

        titulo = TextMobject("\\sc Mechanisms Animations.", color=WHITE).scale(1.5).to_corner(UL).shift(RIGHT)
        ...
        self.play(Write(titulo), ShowCreation(underline), run_time=1)
        self.play(*[FadeIn(objeto) for objeto in [ancla, base_down]],
        ...
        def update(grupo, alpha):
            ...
            piston = self.posicion_piston(O2.get_center(), dx, theta_3 * 180 / PI, self.a, self.b, self.c,
                                          self.piston_color)
            manivela = Line(biela.get_end(), piston.get_center()).set_stroke(self.manivela_color, self.grosor_linea)
            punto_bm = Dot(biela.get_end(), radius=radio)
            punto_mp = Dot(manivela.get_end(), radius=radio)
    ...
    def posicion_piston(self, origen, theta_2, theta_3, a, b, c, color):
        ...
        piston = Rectangle(color=color, height=1, witdh=1.5).set_fill(color, 0.7).scale(0.7).move_to(
            origen + RIGHT * d + UP * c)
        return piston
