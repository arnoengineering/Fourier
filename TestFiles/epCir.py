...
def transform(f, threshold=0.9):
    ...
    # Select
    inds = np.argsort(-M)
    M_norm = M / M.sum()  # all vals between 0,1
    M_norm = np.cumsum(M_norm[inds])
    N = (M_norm < threshold).sum() + 1  # total = 1
...
def viz_factory(N, F_0, F_sel, M_sel, freq_sel, f):  # f_sel is amp of selected freqences
    points = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--precision', dest='precision', default=0.99,
                        help='The percentage precision in estimating the Fourier transform. In range (0-1). Default 0.99.',
                        type=float)
    parser.add_argument('--nframes', dest='n_frames', default=100,
                        help='The number of frames in the produced GIF. Default=100', type=int)
    parser.add_argument('--time', dest='time', default=10,
                        help='The duration (in seconds) of the produced GIF. Default=10', type=int)
    parser.add_argument('--rate', dest='rate', default=2,
                        help='Downsampling rate for the input image. Higher values give coarser approximation but will likely be faster to complete. Default=2.',
                        type=float)
